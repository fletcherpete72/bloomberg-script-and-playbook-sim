"""
IAG5 service: ndu-crud-patch-file-staging

Calls the dbo.crud_patch_file_staging stored procedure to record a patch file
staging operation in the Itential network device upgrade tracking database.
Replaces the MicrosoftSQL adapter task (task a134, summary: itential_db_insert)
in the eos_file_staging_patch_run_job_batch workflow of the
network_device_upgrade project.

Original SQL (from transformation ts_construct_db_query_patch,
ID 698cdb4ef40aa73eab464fbc):

    EXEC dbo.crud_patch_file_staging
...
and @RESULT is JSON-stringified. This script accepts --result_json (the full
result object) and derives success from result_json["success"].
...
IAG5 service definition (iag5-services.yaml):
    - name: ndu-crud-patch-file-staging
      type: python-script
      filename: ndu_crud_patch_file_staging.py
      working-directory: scripts
      repository: iag5-git-repo
      decorator: ndu-crud-patch-file-staging
      secrets:
        - name: mssql-username
          type: env
...
def main():
    parser = argparse.ArgumentParser(
        description="Call dbo.crud_patch_file_staging stored procedure"
    )
    ...
    if sim_url:
        ...
        req = urllib.request.Request(
            f"{sim_url.rstrip('/')}/mssql/exec/crud_patch_file_staging",
            data=payload,
